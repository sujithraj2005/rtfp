...
scheduler_lock = threading.Lock()  # NEW: Lock to protect scheduler state
...
def start_scheduler_route():
    ...
    with scheduler_lock:  # NEW: Protect scheduler state
...
def shutdown_scheduler(exception=None):
    global scheduler_running, scheduler_thread
    # MODIFIED: Only shut down if explicitly needed (e.g., app termination)
    logger.debug("teardown_appcontext called")
    # Avoid shutting down scheduler on every request
    # Shutdown is handled in __main__ block instead
...
if __name__ == '__main__':
    ...
    try:
        ...
        client.close()  # NEW: Close MongoDB client